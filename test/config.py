STORAGE_TYPE = 'file'
THEME = 'suka'
CACHE_TYPE = 'null'
MODE = 'view-only'  # mixed|api-only|view-only
ENTRIES_PER_PAGE = 5
FEED_COUNT = 10
SHOW_TOC = True
TOC_DEPTH = 3  # 1~6
TOC_LOWEST_LEVEL = 3  # 1~6
ALLOW_SEARCH_PAGES = True
PAGE_SOURCE_ACCESSIBLE = False
KEYWORDS = [
  'example',
  'demo',
  'suka',
  'veripress',
  'theme'
]

# veripress-theme-suka
SUKA_POST_ENTRY_EXCERPT = 150
SUKA_READ_MORE = 'READ MORE'
SUKA_AVATAR = 'img/default_avatar.png'
SUKA_DATE_FORMAT = '%Y-%m-%d'
SUKA_UIUX_SLOGAN = 'My Slogan'
SUKA_NAV_SEARCH_USE = True
SUKA_NAV_SEARCH_LINK = '/_search'
# SUKA_HANABI_ENABLE = True
# SUKA_PRETTIFY_ENABLE = True
# SUKA_PRETTIFY_THEME = 'github'
SUKA_UIUX_PRIMARY_COLOR = '#1AAB8E'
SUKA_UIUX_LINK_COLOR = '#1BA1E6'